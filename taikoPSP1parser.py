import sys, os, json, pyglet, struct
...
window = Window(caption="Taiko PSP1 Chart Viewer", width=width,height=height)
...
def section_parse(data, chunksize, measures):
    ...
    for measure in range(measures):
        __json_measure = ''
        [__measure_offset] = struct.unpack('f', data[measure*chunksize:4+(measure*chunksize)])
        [__measure_bpm] = struct.unpack('f', data[4+(measure*chunksize):8+(measure*chunksize)])
        ...
        __measure_advquota1 = None
        if data[16+(measure*chunksize):20+(measure*chunksize)] != b'\xFF\xFF\xFF\xFF':
            __measure_advquota1 = int.from_bytes(data[16+(measure*chunksize):20+(measure*chunksize)], byteorder='little')

        __measure_masquota1 = None
        if data[20+(measure*chunksize):24+(measure*chunksize)] != b'\xFF\xFF\xFF\xFF':
            __measure_masquota1 = int.from_bytes(data[20+(measure*chunksize):24+(measure*chunksize)], byteorder='little')
        
        __measure_advquota2 = None
        if data[24+(measure*chunksize):28+(measure*chunksize)] != b'\xFF\xFF\xFF\xFF':
            __measure_advquota2 = int.from_bytes(data[24+(measure*chunksize):28+(measure*chunksize)], byteorder='little')

        __measure_masquota2 = None
        if data[28+(measure*chunksize):32+(measure*chunksize)] != b'\xFF\xFF\xFF\xFF':
            __measure_masquota2 = int.from_bytes(data[28+(measure*chunksize):32+(measure*chunksize)], byteorder='little')

        __measure_advquota3 = None
        if data[32+(measure*chunksize):36+(measure*chunksize)] != b'\xFF\xFF\xFF\xFF':
            __measure_advquota3 = int.from_bytes(data[32+(measure*chunksize):36+(measure*chunksize)], byteorder='little')

        __measure_masquota3 = None
        if data[36+(measure*chunksize):40+(measure*chunksize)] != b'\xFF\xFF\xFF\xFF':
            __measure_masquota3 = int.from_bytes(data[36+(measure*chunksize):40+(measure*chunksize)], byteorder='little')
        
        __measure_definition_offset_one_nor = int.from_bytes(data[40+(measure*chunksize):44+(measure*chunksize)], byteorder='little')
        __measure_definition_length_one_nor = int.from_bytes(data[44+(measure*chunksize):48+(measure*chunksize)], byteorder='little')
        #__measure_definition_remain_one_nor = int.from_bytes(data[48+(measure*chunksize):52+(measure*chunksize)], byteorder='little')
        __measure_definition_offset_one_adv = int.from_bytes(data[52+(measure*chunksize):56+(measure*chunksize)], byteorder='little')
        __measure_definition_length_one_adv = int.from_bytes(data[56+(measure*chunksize):60+(measure*chunksize)], byteorder='little')
        #__measure_definition_remain_one_adv = int.from_bytes(data[60+(measure*chunksize):64+(measure*chunksize)], byteorder='little')
        __measure_definition_offset_one_mas = int.from_bytes(data[64+(measure*chunksize):68+(measure*chunksize)], byteorder='little')
        __measure_definition_length_one_mas = int.from_bytes(data[68+(measure*chunksize):72+(measure*chunksize)], byteorder='little')
        #__measure_definition_remain_one_mas = int.from_bytes(data[72+(measure*chunksize):76+(measure*chunksize)], byteorder='little')
        __measure_definition_offset_two_nor = int.from_bytes(data[76+(measure*chunksize):80+(measure*chunksize)], byteorder='little')
        __measure_definition_length_two_nor = int.from_bytes(data[80+(measure*chunksize):84+(measure*chunksize)], byteorder='little')
        #__measure_definition_remain_two_nor = int.from_bytes(data[84+(measure*chunksize):88+(measure*chunksize)], byteorder='little')
        __measure_definition_offset_two_adv = int.from_bytes(data[88+(measure*chunksize):92+(measure*chunksize)], byteorder='little')
        __measure_definition_length_two_adv = int.from_bytes(data[92+(measure*chunksize):96+(measure*chunksize)], byteorder='little')
        #__measure_definition_remain_two_adv = int.from_bytes(data[96+(measure*chunksize):100+(measure*chunksize)], byteorder='little')
        __measure_definition_offset_two_mas = int.from_bytes(data[100+(measure*chunksize):104+(measure*chunksize)], byteorder='little')
        __measure_definition_length_two_mas = int.from_bytes(data[104+(measure*chunksize):108+(measure*chunksize)], byteorder='little')
        #__measure_definition_remain_two_mas = int.from_bytes(data[108+(measure*chunksize):112+(measure*chunksize)], byteorder='little')
        __json_measure = json.dumps({'MOF':__measure_offset, 'BPM':__measure_bpm, 'BAR':__measure_barline, 'GOG':__measure_gogotime, 'AQ1':__measure_advquota1, 'MQ1':__measure_masquota1, 'AQ2':__measure_advquota2, 'MQ2':__measure_masquota2, 'AQ3':__measure_advquota3, 'MQ3':__measure_masquota3, 'ONO':__measure_definition_offset_one_nor, 'ONL':__measure_definition_length_one_nor, 'OAO':__measure_definition_offset_one_adv, 'OAL':__measure_definition_length_one_adv, 'OMO':__measure_definition_offset_one_mas, 'OML':__measure_definition_length_one_mas, 'TNO':__measure_definition_offset_two_nor, 'TNL':__measure_definition_length_two_nor, 'TAO':__measure_definition_offset_two_adv, 'TAL':__measure_definition_length_two_adv, 'TMO':__measure_definition_offset_two_mas, 'TML':__measure_definition_length_two_mas})
    ...
    for area_index in range(6):
        if chart[1][area_index]:
            for note_index in range(len(chart[1][measure_no][area_index])-1, -1, -1):
                ...
                image_tiles = image.ImageGrid(image.load('notesCS4.png'), 4, 4)
...
def masstextrender():
    ...
    advanced_quota1 = chart[0][measure_number].get('AQ1')
    master_quota1 = chart[0][measure_number].get('MQ1')
    advanced_text1 = "Advanced Quota 1: " + str(advanced_quota1)
    master_text1 = "Master Quota 1:   " + str(master_quota1)
    advanced_quota2 = chart[0][measure_number].get('AQ2')
    master_quota2 = chart[0][measure_number].get('MQ2')
    advanced_text2 = "Advanced Quota 2: " + str(advanced_quota2)
    master_text2 = "Master Quota 2:   " + str(master_quota2)
    advanced_quota3 = chart[0][measure_number].get('AQ3')
    master_quota3 = chart[0][measure_number].get('MQ3')
    advanced_text3 = "Advanced Quota 3: " + str(advanced_quota3)
    master_text3 = "Master Quota 3:   " + str(master_quota3)
    ...
    textrender(advanced_text1, 330, 90)
    textrender(master_text1, 330, 80)
    textrender(advanced_text2, 330, 70)
    textrender(master_text2, 330, 60)
    textrender(advanced_text3, 330, 50)
    textrender(master_text3, 330, 40)
...
def helprender():
    ...
    rect.draw()
    textrender("Taiko PSP1 Chart Viewer v1.0", 80, 285)
...
def statsrender(chart):
    ...
    for __chart_index in range(len(chart[0])):
        __advancedbranch = chart[0][__chart_index].get('AQ1')
        __masterbranch = chart[0][__chart_index].get('MQ1')
